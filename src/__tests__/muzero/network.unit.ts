...
describe('Network Unit Test:', () => {
  ...
  const network = new CoreNet(new MlpNet(factory.config().observationSize, factory.config().actionSpace), 0.0001, 10)
  ...
  test('Check recurrent inference', () => {
    ...
    expect(nor.tfHiddenState.shape).toEqual([3, ...factory.config().observationSize])
  })
})
